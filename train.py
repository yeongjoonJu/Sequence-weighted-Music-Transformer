...
from model.music_transformer import MusicTransformer, MusicDiscriminator
from model.loss import SmoothCrossEntropyLoss, UncertaintyLoss, WGAN_DIV_Loss

from utilities.constants import *
from utilities.device import get_device, use_cuda
from utilities.lr_scheduling import LrStepTracker, get_lr
from utilities.argument_funcs import parse_train_args, print_train_args, write_model_params
from utilities.run_model import train_epoch, eval_model, train_GAN

CSV_HEADER = ["Epoch", "Learn rate", "Avg val loss", "val accuracy", "Avg Eval loss", "Eval accuracy"]
...
def main():
    ...
    if(args.continue_weights is not None):
        if(args.continue_epoch is None):
            ...
            model.load_state_dict(torch.load(args.continue_weights, map_location='cuda:0'))
    ...
    ##### Weighted evaluation loss #####
    gamma = 2.0
    eval_loss_func = nn.CrossEntropyLoss(ignore_index=TOKEN_PAD, reduction='none')
    weights = torch.FloatTensor([gamma*i/args.max_sequence for i in range(args.max_sequence)])
    weights = weights.unsqueeze(0).repeat(args.batch_size, 1).view(-1)
    ...
    # opt = Adam(filter(lambda x: x.requires_grad, list(model.parameters())+list(unc.parameters())), lr=lr, betas=(ADAM_BETA_1, ADAM_BETA_2), eps=ADAM_EPSILON)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Train
        train_epoch(epoch+1, model, train_loader, train_loss_func, opt, lr_scheduler, args.print_modulus, weights=weights)
        # train_GAN(epoch+1, model, model_D, opt,opt_D, train_loader, train_loss_func, \
        #         lr_scheduler, lr_scheduler_D, args.print_modulus)
        ...
        if(new_best):
            with open(best_text, "w") as o_stream:
                ...
                print("Best vval loss:", best_eval_loss, file=o_stream)
        ...
        with open(results_file, "a", newline="") as o_stream:
            writer = csv.writer(o_stream)
            writer.writerow([epoch+1, lr, val_loss, val_acc, eval_loss, eval_acc])
